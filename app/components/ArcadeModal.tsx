...
export function ArcadeModal(props: ArcadeModalProps) {
  return <AnimatePresence>{props.open && <ModalContent {...props} />}</AnimatePresence>;
}
...
function ModalContent({
  ...
  return (
    <FocusScope contain={isPresent} restoreFocus autoFocus>
      <motion.div
      ...
      >
        <motion.div
          ref={modalRef}
          {...motionAriaProps(mergeProps(modalProps, dialogProps))}
        ...
        >
          ...
          >
            {cancelLabel && (
              <ModalButton
                buttonRef={autoFocusActions ? initialFocusRef : undefined}
                label={cancelLabel}
                onClick={onClose}
              />
            )}
            <ModalButton
              buttonRef={!cancelLabel && autoFocusActions ? initialFocusRef : undefined}
              danger={danger}
              label={confirmLabel}
              onClick={onConfirm}
            />
...
function ModalButton({
  ...
  const { buttonProps, ref, state } = useArcadeButton(
    { onPress: onClick, autoFocus: Boolean(buttonRef) },
    buttonRef,
  );
  return (
    <button
      ...
      style={{
        ...
        padding: 0,
        border: 0,
        outline: state.focused ? "4px solid #fff400" : "none",
        outlineOffset: -8,
